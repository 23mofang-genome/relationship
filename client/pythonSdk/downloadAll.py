...
config.set_default(downloadsuffix='.ufile.cn-north-02.ucloud.cn')
...
def download_ufile_by_name_list(file_name_list):
    ...
    def worker(file_name):
        handler.download_file(bucket, file_name, "/data/run/" + os.path.basename(file_name), isprivate=True)

    data = pool.map(worker, file_name_list)

if __name__ == "__main__":
    rt = get_ufile_list(file_name_start_with='/tmp/ucloud/run/')
    print len(rt)
